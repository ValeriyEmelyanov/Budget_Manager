...
class HashMultiset<E> implements Multiset<E> {
    ...
    public void union(Multiset<E> other) {
        ...
        for (var entry : map.entrySet()) {
            int multiplicity = other.getMultiplicity(entry.getKey());
            if (multiplicity == 0) {
                newMap.put(entry.getKey(), entry.getValue());
            } else {
                newMap.put(entry.getKey(), Math.max(entry.getValue(), multiplicity));
            }
        }
        for (var key : other.toSet()) {
            if (!map.containsKey(key)) {
                newMap.put(key, other.getMultiplicity(key));
            }
        }
        map = newMap;
    }
    ...
    public void intersect(Multiset<E> other) {
        ...
        for (var entry : map.entrySet()) {
            int multiplicity = other.getMultiplicity(entry.getKey());
            if (multiplicity > 0) {
                newMap.put(entry.getKey(), Math.min(entry.getValue(), multiplicity));
            }
        }
        map = newMap;
    }
